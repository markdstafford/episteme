...
- [ ] **Story: Session data model — scope and name**
  - [ ] **Task: Add `SessionScope` enum and `name`/`scope` fields to `session.rs`**
    ...
    - **Acceptance criteria**:
      - [ ] `SessionScope` defined and serialises to `{"type":"document","path":"..."}` / `{"type":"workspace"}`
      - [ ] `Session` struct includes `name: String` and `scope: SessionScope`
      - [ ] Round-trip unit test: serialise and deserialise a `Session` with both scope variants
      - [ ] Project compiles with no errors or warnings
    - **Dependencies**: None
  - [ ] **Task: Mirror changes in `session.ts`; update `newSession()` signature**
    ...
    - **Acceptance criteria**:
      - [ ] `SessionScope` exported as `{ type: "document"; path: string } | { type: "workspace" }`
      - [ ] `Session` interface includes `name: string` and `scope: SessionScope`
      - [ ] `newSession()` accepts `scope` and sets `name: ""` on the returned session
      - [ ] All existing call sites of `newSession()` updated to pass a scope
