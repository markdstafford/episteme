...
-- body stores whichever version (raw or AI-enhanced) is currently selected —
-- updated in-place when the user toggles. queue_comment is an upsert.
CREATE TABLE queued_comments (
  ...
  body        TEXT    NOT NULL,                         -- current selected body
  blocking    BOOLEAN NOT NULL DEFAULT FALSE,
  created_at  TEXT    NOT NULL,                         -- ISO 8601
...
create_thread(doc_id: str, quoted_text: str,
              anchor_from: int, anchor_to: int, body: str, blocking: bool) → Thread
  doc_id is guaranteed to exist in frontmatter before this is called
  (frontend writes it if absent). Insert thread + first comment.
  Emits (→ open). If blocking = true, immediately also emits (→ blocking).
...
queue_comment(id: str, thread_id: str | null, doc_id: str | null,
              ...
              body: str, blocking: bool, created_at: str, expires_at: str) → ()
  Upsert a queued comment by id. Called on initial staging and on body
  updates (e.g. user toggles between raw and AI-enhanced text).
  For immediate commit (e.g. AI fix acceptance), set expires_at = now()
  and call commit_comment immediately after.
